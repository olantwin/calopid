...
def main():
    ...
    y_test = le.transform(np.abs(events[args.target]))[:((n_events//args.batch_size)*args.batch_size)]
    ...
    ### roc_curve ###
    fig, ax = plt.subplots(figsize=(6,4))
    RocCurveDisplay.from_predictions(y_test, y_pred, plot_chance_level=True, ax=ax, figure=fig)
